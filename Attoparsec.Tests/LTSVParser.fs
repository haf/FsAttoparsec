module Attoparsec.Tests.LTSVTest

open Expecto
open Expecto.Flip
open Attoparsec
open Helper
open Attoparsec.String

type Record = Map<string, string>
type LTSV = Record list

[<Tests>]
let ``field test`` =
  testList "LTSV" [
    // This is how you define a parser for the LTSV format.
    let intToCharStr i = i |> char |> string

    // see http://ltsv.org/
    let label = takeWhile1 (string >> isMatch @"[0-9A-Za-z_\.-]")
    let fieldValue = takeWhile (string >> isMatch "[\x01-\x080\x0B\x0C\x0E-\xFF]")
    let field =
      (fun k v -> (BMPString.toString k, BMPString.toString v))
      <!> label <*> (pstring (intToCharStr 0x3A) *> fieldValue)
    let record: Parser<_, Record> =
      Map.ofList <!> (sepBy field (pstring (intToCharStr 0x09)))
    let nl = opt (pstring (intToCharStr 0x0D)) *> pstring (intToCharStr 0x0A)
    let ltsv: Parser<_, LTSV> = sepBy record nl
    let parseField input = input |> parse field |> ParseResult.feed ""
    let parseRecord input = input |> parse record |> ParseResult.feed ""
    let parseLTSV input = input |> parse ltsv |> ParseResult.feed ""

    let test fn input message expected =
      match fn input with
      | Done (_, actual) ->
        actual |> Expect.equal message expected
      | _ ->
        Tests.failtest "Didn't successfully parse."

    yield testCase "field" <| fun () ->
      let expected = ("host", "127.0.0.1")
      let input = "host:127.0.0.1"
      test parseField input "Should parse to tuples" expected

    yield testCase "record" <| fun () ->
      let expected =
        [
          "host", "127.0.0.1";
            "ident", "-";
            "user", "frank";
            "time", "[10/Oct/2000:13:55:36 -0700]";
            "req" ,"GET /apache_pb.gif HTTP/1.0";
            "status", "200";
            "size", "2326";
            "referer", "http://www.example.com/start.html";
            "ua", "Mozilla/4.08 [en] (Win98; I ;Nav)";
        ]
        |> Map.ofList
      let input = "host:127.0.0.1	ident:-	user:frank	time:[10/Oct/2000:13:55:36 -0700]	req:GET /apache_pb.gif HTTP/1.0	status:200	size:2326	referer:http://www.example.com/start.html	ua:Mozilla/4.08 [en] (Win98; I ;Nav)"
      test parseRecord input "Should parse tuples" expected

    yield testCase "list of Map" <| fun () ->
      let expected =
        [
          [
            "title", "test1";
            "user", "user1";
          ] |> Map.ofList;
          [
            "title", "test2";
            "user", "user2";
          ] |> Map.ofList;
        ]
      let input = "title:test1	user:user1\ntitle:test2	user:user2"
      test parseLTSV input "Should parse to a list of Map" expected
  ]