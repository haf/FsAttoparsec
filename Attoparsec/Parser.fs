...
module Parser =
  ...
  /// Bind. The LHS parser must succeed. `f` must return another
  /// parser based on the LHS's successful parse.
  let inline (>>=) p f = bind f p

  /// Map. The LHS parser must succeed. `f` returns the plain value
  /// based on the LHS's successful parse.
  let inline (|>>) p f = map f p

  /// MapValue. The LHS parser must succeed. `x` is the value to replace
  /// the parser state with.
  ...
  let parseOnly skip (m: Monoid<_>) (parser: Parser<_, _>) input =
    ...
    | Done (_, a) ->
      Choice1Of2 a
    | Partial _ ->
      Choice2Of2 "parseOnly: Parser returned a partial result"
